        ...
        NavigationView {
            List {
                Image("about")
                    ...
                    .scaledToFit()
                Section {
                    Link(destination: URL(string: WebLink.rateUs.rawValue)!, label: { //відкрити Safari і там цей лінк
            ...
            .sheet(item: $link) { item in
                if let url = URL(string: item.rawValue) {
                    // WebView(url: url) //use WebKit
                    SafariView(url: url) //use SafariServices
                }
            }
